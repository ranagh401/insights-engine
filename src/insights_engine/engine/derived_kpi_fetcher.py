"""Fetch portfolio-level KPI values from Power BI (Portal: configkpisclientportal only)."""
...
# Portal: no fixed Client tile allow-list — load all KPIs from configkpisclientportal when omitted.
...
async def resolve_portal_kpi_names(
    ...
    """KPI allow-list for portal tiles: explicit names, else all configkpisclientportal rows."""
    if kpi_names:
...
async def _default_portal_kpi_names(db: AsyncSession) -> tuple[str, ...]:
    """All configured base KPIs (Portal uses configkpisclientportal only)."""
...
async def _load_portal_kpi_configs(
    ...
    """Resolve measure metadata from ``configkpisclientportal``."""
    if not kpi_names:
        return {}
    ...
    for name in kpi_names:
        ...
        out[name] = {
            ...
            "measure": row.measure,
            "config_source": "configkpisclientportal",
        }
    return out
...
async def _resolve_portfolio_slice_filters(
    ...
    for dim_name in PORTAL_KPI_SLICE_FILTER_DIMENSIONS:
        ...
        if dim_ref is None:
            raise ValueError(
                f"Slice dimension '{dim_name}' is not mapped in config_dimensionsreunityportal"
            )
...
def _mandatory_portfolio_product_lead_filter() -> tuple[
    ...
    """Portal fork: no hard-coded Client Product_Lead=Bath portfolio filter."""
    return [], []
...
async def fetch_all_derived_kpi_values(
    ...
    *,
    ...
    """Evaluate portfolio KPIs in PBI (default: all rows in configkpisclientportal)."""
    ...
    if not allow:
        return {
            ...
            "period": None,
            "slice_filters": [],
            "message": "No KPIs found in configkpisclientportal.",
        }
    ...
    for name in allow:
        ...
        if not cfg.get("config_source"):
            entry["error"] = (
                f"KPI '{name}' not found in configkpisclientportal"
            )
            meta.append(entry)
            continue
        if not measure:
