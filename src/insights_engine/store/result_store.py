...
MAIN_INSIGHTS_Portal_TABLE = "maininsightsportal"
MAIN_INSIGHTS_DEV_TABLE = "main_insightsdev"
MAIN_INSIGHTS_LEGACY_TABLE = "main_insights"
MAIN_INSIGHTS_DEFAULT_TABLE = MAIN_INSIGHTS_Portal_TABLE
MAIN_INSIGHTS_ALLOWED_TABLES = frozenset(
    {
        MAIN_INSIGHTS_Portal_TABLE,
...
MAIN_INSIGHTS_TABLE_QUERY_HELP = (
    f"Optional target table: {SCHEMA}.{MAIN_INSIGHTS_Portal_TABLE} (Portal default), "
...
def normalize_main_insights_table_name(table_name: str | None) -> str:
    ...
    Accepted values:
    - ``maininsightsportal`` / ``main_insights`` / ``main_insightsdev``
    - ``insights.<table>`` variants
    - ``None`` / empty -> Portal default ``maininsightsportal``
    """
...
maininsightsportal_table = _build_main_insights_table(
    MAIN_INSIGHTS_Portal_TABLE, "ix_maininsightsportal_run"
)
...
# Same as finalportal, but one row per (sales rep, KPI-card group). Rep-level narrative is
# built from signal_log, which is the only place sales_rep-sliced data exists —
# maininsightsportal carries no dimension_name='sales_rep' rows.
finalportalsalesrep_table = Table(
    "finalportalsalesrep",
    metadata,
    ...
    Column("updated_at", DateTime(timezone=True), server_default=text("now()")),
    Index("ix_finalportalsalesrep_rep", "sales_rep"),
)
...
MAIN_INSIGHTS_TABLE_MAP: dict[str, Table] = {
    MAIN_INSIGHTS_Portal_TABLE: maininsightsportal_table,
...
class ResultStore:
    ...
    async def list_config_kpi_names(self) -> tuple[str, ...]:
        """All ``kpiname`` values from insights.configkpisclientportal (portal KPI allow-list)."""
    ...
    async def update_all_signal_job_periods(
        ...
        """Set filters.period.{start,end} on every config_signaljobsclientportal row.
        ...
        sql = text(
            f"""
            UPDATE {SCHEMA}.config_signaljobsclientportal
    ...
    async def get_finalportalsalesrep(
        ...
        if not rep or not grp:
            return None
        async with self._sf() as session:
            stmt = select(finalportalsalesrep_table).where(
                func.lower(finalportalsalesrep_table.c.sales_rep) == rep.lower(),
                func.lower(finalportalsalesrep_table.c.group_name) == grp,
            )
            row = (await session.execute(stmt)).mappings().first()
        return _mapping_to_jsonable(row) if row is not None else None

    async def upsert_finalportalsalesrep(
        self,
        sales_rep: str,
        group_name: str,
        *,
        ...
        async with self._sf() as session:
            stmt = insert(finalportalsalesrep_table).values(**values)
            stmt = stmt.on_conflict_do_update(
                index_elements=[
                    finalportalsalesrep_table.c.sales_rep,
                    finalportalsalesrep_table.c.group_name,
                ],
                set_={
    ...
    async def upsert_finalportal(
        ...
        *,
        ...
        Counterpart to :meth:`get_finalportal_by_group`, which serves these rows verbatim
        ...
        async with self._sf() as session:
            stmt = insert(finalportal_table).values(**values)
            stmt = stmt.on_conflict_do_update(
                index_elements=[finalportal_table.c.group_name],
                set_={
                    "executive_summary": stmt.excluded.executive_summary,
                    "insight": stmt.excluded.insight,
                    "recommended_action": stmt.excluded.recommended_action,
    ...
    async def get_finalportal_by_group(self, group_name: str) -> dict[str, Any] | None:
        """Precomputed executive summary row for a KPI-card group, or None."""
        key = (group_name or "").strip().lower()
        if not key:
            return None
        async with self._sf() as session:
            stmt = select(finalportal_table).where(
                func.lower(finalportal_table.c.group_name) == key
            )
            row = (await session.execute(stmt)).mappings().first()
        return _mapping_to_jsonable(row) if row is not None else None
