...
class ResultStore:
    ...
    async def list_config_kpi_names(self) -> tuple[str, ...]:
        """All ``kpiname`` values from insights.configkpisclientcrm (portal KPI allow-list)."""
    ...
    async def update_all_signal_job_periods(
        ...
        """Set filters.period.{start,end} on every config_signaljobsclientcrm row.
        ...
        sql = text(
            f"""
            UPDATE {SCHEMA}.config_signaljobsclientcrm
