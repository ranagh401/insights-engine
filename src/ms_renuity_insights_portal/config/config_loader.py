...
Reads CRM config tables from PostgreSQL via SQLAlchemy async sessions
and returns Pydantic domain models.  Results are cached with a 15-minute TTL.

CRM fork: KPI definitions are loaded from ``configkpisrenuitycrm`` only (no derived-KPI table).
"""
...
from ..models.crm_config_tables import (
...
class ConfigLoader:
    ...
    async def get_signal_definitions(
        ...
        """Load signal definitions from config_signalsrenuitycrm by name."""
