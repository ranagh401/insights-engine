...
Monthly: ``1-30 Apr``
Weekly: ``22-28 Jun`` (CRM default week; was ``3-9 May`` on Renuity)
"""
...
# CRM portal fallback when main_insights has no run (weekly only).
...
def split_portal_insight_rows(
    ...
    """Split insights into portal monthly (``1-30 Apr``) and weekly (CRM ``22-28 Jun``) buckets."""
...
def default_portal_period_window(bucket: PortalPeriodType) -> dict[str, Any] | None:
    """Fixed CRM windows when main_insights cannot supply dates."""
...
def resolve_portal_period_window(
    ...
    """Prefer main_insights window; fall back to CRM defaults (weekly only)."""
