"""Fetch portfolio-level KPI values from Power BI (CRM: configkpisrenuitycrm only)."""
...
# CRM: no fixed Renuity tile allow-list — load all KPIs from configkpisrenuitycrm when omitted.
...
async def resolve_portal_kpi_names(
    ...
    """KPI allow-list for portal tiles: explicit names, else all configkpisrenuitycrm rows."""
    if kpi_names:
...
async def _default_portal_kpi_names(db: AsyncSession) -> tuple[str, ...]:
    """All configured base KPIs (CRM uses configkpisrenuitycrm only)."""
...
async def _load_portal_kpi_configs(
    ...
    """Resolve measure metadata from ``configkpisrenuitycrm``."""
    if not kpi_names:
        return {}
    ...
    for name in kpi_names:
        ...
        out[name] = {
            ...
            "measure": row.measure,
            "config_source": "configkpisrenuitycrm",
        }
    return out
...
def _mandatory_portfolio_product_lead_filter() -> tuple[
    ...
    """CRM fork: no hard-coded Renuity Product_Lead=Bath portfolio filter."""
    return [], []
...
async def fetch_all_derived_kpi_values(
    ...
    *,
    ...
    """Evaluate portfolio KPIs in PBI (default: all rows in configkpisrenuitycrm)."""
