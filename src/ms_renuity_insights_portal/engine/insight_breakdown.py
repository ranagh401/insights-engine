...
def breakdown_dimensions_for_parent(parent_dimension: str | None) -> tuple[str, ...]:
    """CRM breakdown dimensions for an insight parent slice.
...
async def fetch_insight_breakdown(
    *,
    ...
    """Load one main insight and return KPI values by CRM breakdown dimensions."""
    ...
    if not portfolio_mode:
        ...
        if parent_ref is None:
            raise ValueError(
                f"Parent dimension '{parent_dim_name}' is not mapped in config_dimensionsreunitycrm"
            )
    ...
    for child_dim_name in child_dims:
        ...
        if child_ref is None:
            block["error"] = f"Dimension '{child_dim_name}' is not mapped in config_dimensionsreunitycrm"
