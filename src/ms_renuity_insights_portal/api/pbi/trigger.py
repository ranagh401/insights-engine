...
class TriggerSignalsRequest(BaseModel):
    ...
    dimension_name: Optional[str] = Field(
        ...
        description=(
            ...
            "e.g. ``Division``. Must match values in ``config_signaljobsrenuitycrm.dimensions``."
        ),
    )
...
class TriggerWhysByKpisRequest(BaseModel):
    ...
    kpi_names: list[str] = Field(
        ...
        description=(
            ...
            "(case-insensitive), e.g. metrics whose ``configkpidependenciesrenuitycrm`` rows you just updated."
        ),
    )
    delete_existing_why_rows: bool = Field(
        True,
...
async def trigger_whys_batch(
    ...
    Typical use: after fixing ``configkpidependenciesrenuitycrm`` or thresholds, re-run WHY
